...
Page.getLayout = function getLayout(page: any) {
  return <AdminLayout title="Master Khusus - Tenor Mikro">{page}</AdminLayout>;
};
...
export default function Page() {
  ...
  const {
    ...
    total: totalTenor,
  } = MasterKhusuRepository.hooks.tenorMikro.useList({
  ...
  const onAddButton = () => {
    push({
      pathname: `tenor-mikro/form/new`,
    });
  };

  const onEditButton = (id: string) => {
    push({
      pathname: `tenor-mikro/form/${id}`,
    });
  };
                  ...
                  {dataTenor.map((item, index) => {
                    return (
                      <Table.Tr key={item.id_inc}>
                        ...
                        <Table.Td align="right">{item.tenor}</Table.Td>
                        <Table.Td>
                          <Group>
                            <Button
                              variant="outline"
