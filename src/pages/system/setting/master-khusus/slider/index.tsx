...
Page.getLayout = function getLayout(page: any) {
  return <AdminLayout title="Master Khusus - Slider">{page}</AdminLayout>;
};
...
export default function Page() {
  ...
  const {
    ...
    total: totalSlider,
    mutate: reloadDataSlider,
  } = MasterKhusuRepository.hooks.slider.useList({
  ...
  const onAddButton = () => {
    push({
      pathname: `slider/form/new`,
    });
  };

  const onEditButton = (id: string) => {
    push({
      pathname: `slider/form/${id}`,
    });
  };
                  ...
                  {dataSlider.map((item, index) => {
                    return (
                      <Table.Tr key={item.id_inc}>
                        ...
                        <Table.Td>{item.judulSlider}</Table.Td>
                        <Table.Td>{item.image}</Table.Td>
                        <Table.Td>{item.keteranganSlider}</Table.Td>
                        <Table.Td>
                          <Group>
                            <Button
                              variant="outline"
