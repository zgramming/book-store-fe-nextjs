...
Page.getLayout = function getLayout(page: any) {
  return <AdminLayout title="Master Student">{page}</AdminLayout>;
};
...
export default function Page() {
  ...
  const {
    data: masterStudentData,
    isLoading,
    mutate: reloadData,
    total,
  } = MasterStudentRepository.hooks.useList({
  ...
  const onAddButton = () => {
    push({
      pathname: 'master-student/form/new',
    });
  };

  const onEditButton = (id: string) => {
    push({
      pathname: `master-student/form/${id}`,
    });
  };

  const onDeleteHandler = async (id: string) => {
    try {
      await MasterStudentRepository.api.delete(id);
      notifications.show({
        title: 'Success',
        color: 'green',
          ...
          <Stack gap={'md'} id="table">
            ...
            <Table.ScrollContainer minWidth={500}>
              <Table verticalSpacing={'md'}>
                ...
                <tbody>
                  {masterStudentData.map((item, index) => {
                    return (
                      <Table.Tr key={item.id}>
                        <Table.Td>{index + 1}</Table.Td>
                        <Table.Td>{item.name}</Table.Td>
                        <Table.Td>{item.nim}</Table.Td>
