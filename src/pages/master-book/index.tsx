...
Page.getLayout = function getLayout(page: any) {
  return <AdminLayout title="Master Book">{page}</AdminLayout>;
};
...
export default function Page() {
  ...
  const {
    ...
    mutate: reloadData,
    total,
  } = MasterBookRepository.hooks.useList({
  ...
  const onAddButton = () => {
    push({
      pathname: 'master-book/form/new',
    });
  };

  const onEditButton = (id: string) => {
    push({
      pathname: `master-book/form/${id}`,
    });
  };

  const onDeleteHandler = async (id: string) => {
    try {
      await MasterBookRepository.api.delete(id);
      notifications.show({
        title: 'Success',
        color: 'green',
